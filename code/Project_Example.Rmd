...
ggplot(data = top5, aes(x = Country_name,y = Happiness_score)) +
  geom_point() + 
  labs(x = "Country", y = "Happiness Score", main = "Happiness Score among Top Four Countries")
```

I felt that this plot could be improved. So first I used I made `Country_name` a factor and then reordered it by the `Happiness_score` variable. I also make the point size larger and used the `theme_bw()` to remove the grey plot background.
...
ggplot(data = int_Country, aes(x = Country_name, y = Happiness_score)) +
  geom_point(size=3) + 
  labs(x = "Country", y = "Happiness Score", main = "Happiness Score among Top Four Countries") +
  theme_bw()
```

I can see from this plot that among the four countries of interest, the United States ranked highest. Mexico ranked second, China third, and India fourth.
...
I then used `ggplot()`  and `geom_boxplot()` to create a boxplot of the happiness scores for the countries by regions. I decided to use `coord_flip()` to flip the coordinates to make it easier to read the different region names. I colored the boxplots but using the `fill` argument within the `aes` specifications and used a qualitative RColorBrewer palette. I changed the style of the plot with `theme_linedraw()` and I removed the legend using `theme(legend.position = "none")`.

```{r}

ggplot(data=Happiness, aes(x=`Regional indicator`, y=Happiness_score, fill=`Regional indicator`)) +
  geom_boxplot() +
  theme_linedraw() +
  theme(legend.position = "none") +
  coord_flip() + 
  labs(x="Country", y="Happiness Score", main="Happiness Score by Region") +
...
1. The student described **where** the data came from including how the instructor could access the data. (2 / 2 points)
2. The student described information about the data (what the variables mean) and how the data was originally created. (2 / 2 points)
3. The student performed more than 3 data wrangling methods described in the course, such as arranging data, filtering data, renaming variables, and creating a new variable (`rank`). (6 / 6 points)
4. The student described what steps they did to wrangle the data and why.  (3/3 points)
5. The student made two different kinds of visualizations (scatter/point and boxplot). The plots are "pretty". (8 / 8 points)
6. The student performed a simple analysis on the data to compare two regions with a t-test.  (4 / 4  points)
7. The student described the analysis and provided some interpretation of the results. (4 / 4 points)
8. The student had `session_info()` print at the end of the analysis (1 / 1 points)
9. The student made sure that the document knit to create an html file.
Bonus: A function was also created worth 4 points, but this is not required.

Thus student would get full points (30 out of 30). The extra points from the bonus would not be used to account for less points for other aspects of the project, as the student fulfilled all the other aspects required.
